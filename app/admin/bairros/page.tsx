...
export default async function NeighborhoodsPage() {
  const neighborhoods = await db.neighborhood.findMany({ include: { _count: { select: { projects: true } } }, orderBy: { name: "asc" } });
  return <>
    <div className="eyebrow">Catálogo territorial</div><h1>Bairros</h1>
    <p>Cadastre a entidade territorial uma vez e reutilize-a em empreendimentos, páginas locais e conteúdo SEO.</p>
    <form action={createNeighborhoodAction} className="panel form-grid">
      <label>Nome<input name="name" required minLength={2} /></label>
      <label>Imagem principal (URL)<input name="heroImage" type="url" /></label>
      <label className="full">Descrição<textarea name="description" rows={4} /></label>
      <label>Título SEO<input name="seoTitle" maxLength={70} /></label>
