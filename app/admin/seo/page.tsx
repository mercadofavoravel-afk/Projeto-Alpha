...
export default async function SeoMissionControlPage() {
  ...
  const projectScores = projects.map((item) => ({
    id: item.id,
    name: item.name,
    kind: 'Empreendimento',
  ...
  const neighborhoodScores = neighborhoods.map((item) => ({
    id: item.id,
    name: item.name,
    kind: 'Bairro',
    status: 'ENTIDADE',
  ...
  const articleScores = articles.map((item) => ({
    id: item.id,
    name: item.title,
    kind: 'Artigo',
    status: item.publishStatus,
    ...calculateSeoScore({ ...item, name: item.title, description: item.excerpt }),
  }));
  const all = [...projectScores, ...neighborhoodScores, ...articleScores];
  const average = all.length
    ? Math.round(all.reduce((sum, item) => sum + item.score, 0) / all.length)
    : 0;
  ...
  return (
    <>
      ...
      <div className="kpis">
        <div className="kpi">
          <b>{average}</b>Score médio
        </div>
        <div className="kpi">
          <b>{all.length}</b>Entidades auditadas
        </div>
        <div className="kpi">
          <b>{critical.length}</b>Abaixo de 60
        </div>
        <div className="kpi">
          <b>{all.filter((x) => x.score >= 80).length}</b>Prontas para publicar
        </div>
      </div>
      <div className="panel">
              ...
              <tbody>
                {critical.map((item) => (
                  <tr key={`${item.kind}-${item.id}`}>
                    ...
                    <td>
                      {item.checks.find((check) => !check.passed)?.label ?? 'Revisão editorial'}
                    </td>
                  </tr>
                ))}
              </tbody>
