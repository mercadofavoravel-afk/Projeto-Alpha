...
export async function POST(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!["UPLOADED", "FAILED"].includes(book.status)) {
    return NextResponse.json({ error: "Book já está em processamento ou concluído" }, { status: 409 });
  }
  ...
  return NextResponse.json({
    ...updated,
    message: "Processamento enfileirado. Conecte este endpoint a um worker externo antes da produção."
  });
}
