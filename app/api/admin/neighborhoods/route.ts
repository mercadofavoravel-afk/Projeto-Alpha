...
export async function GET() {
  ...
  const data = await db.neighborhood.findMany({ include: { _count: { select: { projects: true } } }, orderBy: { name: "asc" } });
...
export async function POST(request: Request) {
  ...
  const neighborhood = await db.neighborhood.create({ data: { ...parsed.data, heroImage: parsed.data.heroImage || null, slug } });
