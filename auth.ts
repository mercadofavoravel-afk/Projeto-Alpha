...
export async function login(emailInput: string, password: string) {
  ...
  if (failed >= maxAttempts) return { ok: false as const, reason: "RATE_LIMITED" as const };
  ...
  await db.session.create({ data: { tokenHash: hash(token), userId: user.id, expiresAt, ipHash, userAgent } });
...
export async function requireApiPermission(permission: string) {
  ...
  if (!hasPermission(user.role, permission)) return { ok: false as const, status: 403, error: "Sem permissão" };
  return { ok: true as const, user };
}
...
export async function createPasswordReset(emailInput: string) {
  ...
  await db.passwordResetToken.create({
    data: { tokenHash: hash(token), userId: user.id, expiresAt: new Date(Date.now() + 30 * 60_000) }
  });
  return token;
}
