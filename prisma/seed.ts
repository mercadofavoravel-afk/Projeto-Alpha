...
async function main() {
  ...
  for (const s of data) {
    const n = await prisma.neighborhood.upsert({
      ...
      create: { slug: slugify(s.neighborhood), name: s.neighborhood },
    });
    ...
    for (const cName of s.collections) {
      const c = await prisma.collection.upsert({
        ...
        update: { name: cName },
        create: { slug: slugify(cName), name: cName },
      });
      await prisma.projectCollection.upsert({
        where: { projectId_collectionId: { projectId: p.id, collectionId: c.id } },
        update: {},
        create: { projectId: p.id, collectionId: c.id },
      });
    }
    for (const t of s.types) {
      const exists = await prisma.typology.findFirst({ where: { projectId: p.id, name: t } });
      if (!exists) await prisma.typology.create({ data: { name: t, projectId: p.id } });
    }
  }
}
main()
